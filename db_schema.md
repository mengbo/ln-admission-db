...
batch 为批次字段，表示录取批次；
first 为首选科目字段，可以是历史或物理，区分考生为历史类(即文科）还是物理类(即理科）；
inst_code 为院校编码字段，为主管部门统一编码，一般不变化；
inst 为院校字段，即院校名称；
major_code 为专业编码字段，为主管部门统一编码，偶有变化；
major 为专业字段，即简单描述招生专业；
second 为次选科目字段，为招生院校对考生次选科目要求；
plan_num 为招生计划数字段，即该院校此专业在辽宁计划招生人数。

## 一分一段表

在高考出成绩后，主管部门会发布考生考试成绩的一分一段表，通过此表可以知道分数所对应的省内排名，由于高考是以成绩排名从高到底按照考生志愿进行录取的，所以考生排名才是决定考试录取的关键。例如2024年物理类701分省内排名为52名，700分省内排名为65名；2023年物理类700分省内排名为49名，699分省内排名为65名；则2024年的701分，大概相当于2023年的699分。
...
inst 为院校字段，即院校名称；
major_code 为专业编码字段，为主管部门统一编码，偶有变化；
major 为专业字段，即招生专业名称，详细描述了该专业的信息；
